...
The modeling compared trees and ensembles, using hyperparameter optimization and sampling strategies. The \textbf{Extra Trees} (without hybrid sampling) model was chosen as \textbf{Model}. The \textbf{Adaptive Boosting} (without hybrid sampling) model, chosen as \textbf{AltModel}, maintained good performance even when excluding the dominant features. 

Furthermore, \textbf{XAIModel} and \textbf{AltXAIModel} represent the explainable Decision Trees for the dataset with and without the dominant features. 
